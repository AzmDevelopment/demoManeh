...
import {
  ...
  UntypedFormGroup,
  Validators
} from "./chunk-KA6SXBIR.js";
import {
  DomSanitizer
} from "./chunk-CNLYICNS.js";
import {
  AsyncPipe,
  CommonModule
} from "./chunk-GWW5VEP7.js";
import {
...
function clone(value) {
  if (!isObject(value) || (0, import_rxjs.isObservable)(value) || value instanceof TemplateRef || /* instanceof SafeHtmlImpl */
...
var FieldExpressionExtension = class {
  onPopulate(field) {
    ...
    const evalExpr = (key, expr) => {
      ...
      } else if (expr instanceof import_rxjs.Observable) {
        field._expressions[key] = {
          value$: expr.pipe((0, import_operators.tap)((v) => {
    ...
    return {
      callback: (ignoreCache) => {
        try {
          ...
          if (ignoreCache || currentValue !== exprValue && (!isObject(exprValue) || (0, import_rxjs.isObservable)(exprValue) || JSON.stringify(exprValue) !== JSON.stringify(currentValue))) {
...
var CoreExtension = class {
  ...
  initRootOptions(field) {
    ...
    if (!options.fieldChanges) {
      defineHiddenProp(options, "fieldChanges", new import_rxjs.Subject());
    }
      ...
      if (c) {
        ...
        if (null === c.asyncValidator && this.hasValidators(field, "_asyncValidators")) {
          c.setAsyncValidators(() => {
            ...
            return v ? v(c) : (0, import_rxjs.of)(null);
          });
          markForCheck = true;
        }
        if (markForCheck) {
...
var FieldValidationExtension = class {
  ...
  wrapNgValidatorFn(field, validator, validatorName) {
    ...
    return (control) => {
      ...
      if ((0, import_rxjs.isObservable)(errors)) {
        return errors.pipe((0, import_operators.map)((v) => this.handleResult(field, validatorName ? !!v : v, validatorOption)));
      }
  ...
  triggerHook(name, changes) {
    ...
    if (this.field?.hooks?.[name]) {
      if (!changes || changes.field) {
        ...
        if ((0, import_rxjs.isObservable)(r) && ["onInit", "afterContentInit", "afterViewInit"].indexOf(name) !== -1) {
  ...
  fieldChanges(field) {
    ...
    for (const key of Object.keys(field._expressions || {})) {
      const expressionObserver = observe(field, ["_expressions", key], ({
        ...
        if ((0, import_rxjs.isObservable)(currentValue.value$)) {
    ...
    if (field.formControl && !field.fieldGroup) {
      const control = field.formControl;
      let valueChanges = control.valueChanges.pipe((0, import_operators.map)((value) => {
        field.parsers?.map((parserFn) => value = parserFn(value, field));
        if (!Object.is(value, field.formControl.value)) {
          field.formControl.setValue(value);
        }
        return value;
      }), (0, import_operators.distinctUntilChanged)((x, y) => {
      ...
      if (control.value !== getFieldValue(field)) {
        valueChanges = valueChanges.pipe((0, import_operators.startWith)(control.value));
      }
      const {
        updateOn,
        debounce
      ...
      if ((!updateOn || updateOn === "change") && debounce?.default > 0) {
        valueChanges = valueChanges.pipe((0, import_operators.debounceTime)(debounce.default));
      }
...
var FormlyForm = class _FormlyForm {
  ...
  valueChanges() {
    ...
    const valueChanges = this.field.options.fieldChanges.pipe((0, import_operators.filter)(({
      field,
      type
    }) => hasKey(field) && type === "valueChanges"), (0, import_operators.switchMap)(() => isNoopNgZone(this.ngZone) ? (0, import_rxjs.of)(null) : this.ngZone.onStable.asObservable().pipe((0, import_operators.take)(1)))).subscribe(() => this.ngZone.runGuarded(() => {
      this.checkExpressionChange();
...
var FormlyValidationMessage = class _FormlyValidationMessage {
  ...
  ngOnChanges() {
    ...
    this.errorMessage$ = (0, import_rxjs.merge)(this.field.formControl.statusChanges, !this.field.options ? (0, import_rxjs.of)(null) : this.field.options.fieldChanges.pipe((0, import_operators.filter)(({
      field,
      type,
      property
    }) => {
      return field === this.field && type === "expressionChanges" && (property.indexOf("validation") !== -1 || EXPR_VALIDATORS.indexOf(property) !== -1);
    }))).pipe((0, import_operators.startWith)(null), (0, import_operators.switchMap)(() => (0, import_rxjs.isObservable)(this.errorMessage) ? this.errorMessage : (0, import_rxjs.of)(this.errorMessage)));
  }
  get errorMessage() {
...
//# sourceMappingURL=chunk-2IMWCF3U.js.map
